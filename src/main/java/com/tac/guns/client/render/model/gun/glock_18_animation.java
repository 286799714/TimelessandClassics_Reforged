 ...
 */
public class glock_18_animation extends DeconstructedGunModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrices, MultiBufferSource renderBuffer, int light, int overlay) {
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getModelComponent(skin, BODY), Glock18AnimationController.INDEX_BODY, transformType, matrices);
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getModelComponent(skin, BODY), Glock18AnimationController.INDEX_MAG, transformType, matrices);
        ...
        if (controller.getAnimationFromLabel(GunAnimationController.AnimationLabel.RELOAD_NORMAL).equals(controller.getPreviousAnimation())) {
            matrices.pushPose();
            {
                controller.applySpecialModelTransform(getModelComponent(skin, BODY), Glock18AnimationController.INDEX_EXTRA_MAG, transformType, matrices);
                renderMag(stack, matrices, renderBuffer, light, overlay, skin);
            }
            matrices.popPose();
        }
        ...
        //Always push
        matrices.pushPose();
        controller.applySpecialModelTransform(getModelComponent(skin, BODY), Glock18AnimationController.INDEX_SLIDE, transformType, matrices);
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getModelComponent(skin, BODY), Glock18AnimationController.INDEX_BULLET, transformType, matrices);
