...
{
    private static final RenderType BULLET_TRAIL = RenderType.makeType(Reference.MOD_ID + ":projectile_trail", DefaultVertexFormats. POSITION_COLOR, GL_QUADS, 256, true, true,
            RenderType.State.getBuilder().cull(CULL_DISABLED).alpha(DEFAULT_ALPHA).transparency(TRANSLUCENT_TRANSPARENCY)/*.diffuseLighting(RenderState.DIFFUSE_LIGHTING_DISABLED).overlay(OVERLAY_ENABLED)*/.build(false));
    private static final RenderType SCREEN =
            RenderType.makeType(Reference.MOD_ID + ":screen_texture", DefaultVertexFormats.ENTITY, GL_QUADS, 1024, true, false, RenderType.State.getBuilder().texturing(ScreenTextureState.instance()).lightmap(LIGHTMAP_ENABLED).overlay(OVERLAY_ENABLED).cull(RenderState.CULL_ENABLED).build(false)); //256
    ...
    private static final RenderType SCREEN_BLACK =
            RenderType.makeType(Reference.MOD_ID + ":screen_black", DefaultVertexFormats.ENTITY, GL_QUADS, 64, true, false,
                    RenderType.State.getBuilder().texture(new RenderState.TextureState(GunRenderingHandler.MUZZLE_SMOKE_TEXTURE, false, false)).build(false)); //256
